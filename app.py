...
if not os.path.exists(TEMP_DIR):
    ...
    for f in os.listdir(TEMP_DIR):
        ...
        try:
            if os.path.isfile(f_path) and current_time - os.path.getctime(f_path) > 3600:
                os.remove(f_path)
        except Exception:
            pass

st.set_page_config(page_title="音声縮小ツール Ultimate", page_icon="🎛️")
...
def compress_audio_custom(input_file, output_file, duration_sec, settings):
    ...
    cmd = [
        ...
        '-vn',                 # 映像削除
        '-c:a', audio_codec,   # コーデック (aac or libmp3lame)
        '-ac', str(settings['channels']), # チャンネル数 (1 or 2)
        '-ar', '16000',        # サンプリングレート(16kHz固定)
        '-b:a', f"{settings['kbps']}k", # ビットレート
        '-y', output_file
    ]
    ...
    while True:
        ...
        if line:
            ...
            if match:
                ...
                status_text.write(f"🔄 変換中... {int(progress*100)}% (経過: {elapsed_time:.1f}秒)")
...
if uploaded_file:
    ...
    if info:
        ...
        info_items = [
            ...
            ("Ch", f"{'ステレオ' if info['channels'] == 2 else 'モノラル'} ({info['channels']}ch)"),
            ("Bitrate", f"{int(info['bit_rate']/1000) if info['bit_rate'] else '?'} kbps")
        ]
        ...
        with set_col3:
            ...
            default_index = min(range(len(bitrate_options)), key=lambda i: abs(bitrate_options[i] - auto_kbps))
        ...
        if st.button("変換スタート", type="primary"):
            ...
            output_name = os.path.splitext(original_name)[0] + f"_gemini.{selected_format}"
            ...
            success, processing_time = compress_audio_custom(input_path, output_path, info['duration'], settings)
